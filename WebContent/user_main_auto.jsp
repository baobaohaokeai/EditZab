...
<link rel="stylesheet"
	href="${basePath}/bootstrap/css/bootstrap.min.css"></link>
<script type="text/javascript">
	$(function() {
...
<body>
 ...
 --%>
							...
							<c:if test="${!empty m.module.path }">
								<c:choose>
									<c:when test="${fn:contains(m.module.path,'reportAction')}">
										<li><a target="${m.target }"
											href="${basePath }${m.module.path}">${m.name }</a></li>
									</c:when>
									<c:otherwise>
										<li><a target="${m.target }"
							...
							<li class="dropdown"><a href="#" class="dropdown-toggle"
								...
								<ul class="dropdown-menu">
									<c:forEach var="sm" items="${m.children }">
										...
										<c:if test="${!empty sm.module.path }">
											<c:choose>
												<c:when test="${fn:contains(sm.module.path,'reportAction')}">
													<li><a target="${sm.target }"
													href="${basePath }${sm.module.path}">${sm.name }</a></li>
												</c:when>
												<c:otherwise>
													<li><a target="${sm.target }"
